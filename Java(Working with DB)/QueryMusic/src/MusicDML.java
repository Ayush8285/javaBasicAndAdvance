...
public class MusicDML {
    public static void main(String[] args) {
        ...
        //old version Class.forName() is deprecated
//        try {
...
//            System.out.println("Artist was " + (found ? "found" : "not found"));
            ...
            String columnValue = "Bob Dylan";                                                //Neil Young     ,Bob Dylan //for deleting Elf it throws error because it ia a foreign key
...
//        return insertResult;
    ...
    private static boolean deleteRecord(Statement statement, String table,
                                        String columnName, String columnValues)
        throws SQLException{

        String query = "DELETE FROM %s WHERE %s='%s'"
                .formatted(table, columnName, columnValues);
        System.out.println(query);
        statement.execute(query);
        int recordsDeleted = statement.getUpdateCount();
        if(recordsDeleted > 0){
            executeSelect(statement, table, columnName, columnValues);
        }
    ...
    private static boolean updateRecord(Statement statement, String table,
                                        ...
                                        String updatedName, String updatedValues)
            throws SQLException{

        String query = "UPDATE %s SET %s = '%s' WHERE %s='%s'"
                .formatted(table, updatedName, updatedValues, matchedColumn, matchedValue);
        System.out.println(query);
        statement.execute(query);
        int recordsUpdated = statement.getUpdateCount();
        if(recordsUpdated > 0){
            executeSelect(statement, table, updatedName, updatedValues);
        }
    ...
    private static void insertArtistAlbum(Statement statement, String artistName,
        ...
        for (int i = 1; i < songs.length; i++) {
            String songQuery = songInsert.formatted(i ,
